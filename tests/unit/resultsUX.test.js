...
const bundled=await build({stdin:{contents:`import React from 'react';import{renderToStaticMarkup}from'react-dom/server';import Results from './src/screens/ResultsScreen.jsx';import Packing from './src/components/PackingChecklist.jsx';export const renderPlan=(props)=>renderToStaticMarkup(React.createElement(Results,props));export const renderPacking=(packingList)=>renderToStaticMarkup(React.createElement(Packing,{packingList}));`,resolveDir:new URL('../../src/frontend/',import.meta.url).pathname},bundle:true,write:false,format:'cjs',platform:'node',define:{'import.meta.env':'{}'},jsx:'automatic'});
